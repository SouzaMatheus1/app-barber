...
import { Calendar as CalendarIcon, Clock, User, X, CheckCircle2, Plus, Ban, Scissors } from 'lucide-react';
...
export function Agenda() {
  ...
  // States Modal Walk In
  const [walkinProfissional, setWalkinProfissional] = useState<number>(0);
  const [walkinServicosIds, setWalkinServicosIds] = useState<number[]>([]);
  
  // States Autocomplete Cliente
  const [walkinClienteSearch, setWalkinClienteSearch] = useState('');
  const [walkinClienteId, setWalkinClienteId] = useState<number | ''>('');
  ...
  const loadBasics = async () => {
    try {
      ...
      if (resProf.data.length > 0) {
        setWalkinProfissional(resProf.data[0].id);
  ...
  const submitWalkin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!walkinProfissional) return alert('Selecione o profissional');
    if (walkinServicosIds.length === 0) return alert('Selecione pelo menos um serviço.');
    
    // Calcula duraçao total baseada nos itens do catálogo mockado no front
    let totalMinutos = 0;
    walkinServicosIds.forEach(id => {
      const s = servicos.find(x => x.id === id);
      if (s) totalMinutos += s.duracaoMinutos;
    });

    const now = new Date();
    const fim = new Date(now.getTime() + totalMinutos * 60000);
    
    const obsVal = walkinClienteSearch ? `Walk-in Avulso: ${walkinClienteSearch}` : 'Walk In Anônimo';

    try {
      await api.post('/agendamentos', {
        profissionalId: walkinProfissional,
        clienteId: walkinClienteId !== '' ? walkinClienteId : undefined,
        servicosIds: walkinServicosIds,
        dataHoraInicio: now.toISOString(),
        dataHoraFim: fim.toISOString(),
        status: 'EM_ANDAMENTO',
        observacao: walkinClienteId !== '' ? 'Walk In Registrado' : obsVal
      });
      setIsWalkinModalOpen(false);
      setWalkinServicosIds([]);
      setWalkinClienteId('');
      setWalkinClienteSearch('');
      await loadAgenda();
    } catch (error: any) {
      alert(error.response?.data?.error || "Conflito: Barbeiro indisponível no momento.");
    }
  };
  ...
  const submitBlock = async (e: React.FormEvent) => {
    e.preventDefault();
    const now = new Date();
    const fim = new Date(now.getTime() + blockDuracao * 60000);
    ...
    try {
      ...
      alert(error.response?.data?.error || "Conflito ao registrar bloqueio.");
    }
  };

  const toggleServico = (id: number) => {
    setWalkinServicosIds(prev => 
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        >
          <Scissors size={18} /> Walk-In (Na Cadeira Agora)
        </button>
        <button 
                                  ...
                                  return (
                                    <div key={ag.id} className="p-3 rounded-lg border bg-red-950/20 border-red-500/20 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                       <div className="flex flex-col">
                                          ...
                                          <span className="text-xs text-[#E5E5E5]/50 mt-1">Barbeiro: ID {ag.profissionalId} | Motivo: {ag.observacao}</span>
                                       </div>
                                    ...
                                    `}>
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-[#E5E5E5]/80">
                                                ...
                                                <span className="text-xs text-[#E5E5E5]/40">Profissional ID: {ag.profissionalId}</span>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                {ag.servicos.map((s, idx) => (
                                                    <span key={idx} className="text-[10px] sm:text-xs bg-[#1a1a1a] border border-[#D4AF37]/10 px-2 py-1 rounded text-[#E5E5E5]/60 flex items-center gap-1">
                                                       {s.item.nome} <span className="opacity-50 ml-0.5">({s.item.duracaoMinutos}m)</span>
                                                    </span>
                                                ))}
                                            </div>
                                        </div>
      ...
      {isWalkinModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <form onSubmit={submitWalkin} className="bg-[#121212] border border-[#D4AF37]/30 rounded-xl p-6 w-full max-w-md shadow-2xl relative">
            <button type="button" onClick={() => setIsWalkinModalOpen(false)} className="absolute top-4 right-4 text-[#E5E5E5]/50 hover:text-[#E5E5E5]">
              <X size={24} />
            </button>
            <h2 className="text-xl font-bold text-[#D4AF37] mb-6 flex items-center gap-2">
              <Scissors size={20} /> Novo Walk-in Rápido
            </h2>

            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-[#E5E5E5]/80 uppercase">Profissional</label>
                <select required value={walkinProfissional} onChange={e => setWalkinProfissional(Number(e.target.value))} className="w-full mt-1 bg-[#1a1a1a] text-[#E5E5E5] border border-[#D4AF37]/30 rounded-lg px-4 py-2 outline-none">
              ...
              <div className="relative">
                ...
                <input 
                  type="text" 
                  value={walkinClienteSearch} 
                  onChange={e => {
                    setWalkinClienteSearch(e.target.value);
                    setWalkinClienteId(''); // Se ta digitando livremente, tira o ID acoplado
                    setShowClienteDropdown(true);
                  }}
                  onFocus={() => setShowClienteDropdown(true)}
                  onBlur={() => setTimeout(() => setShowClienteDropdown(false), 200)}
                  placeholder="Selecione ou digite um nome avulso" 
                  className={`w-full mt-1 bg-[#1a1a1a] border border-[#D4AF37]/30 px-4 py-2 outline-none transition-colors ${walkinClienteId !== '' ? 'text-[#D4AF37] font-bold' : 'text-[#E5E5E5]'}`} 
                  style={{ borderRadius: showClienteDropdown && clientes.filter(c => c.nome.toLowerCase().includes(walkinClienteSearch.toLowerCase())).length > 0 ? '0.5rem 0.5rem 0 0' : '0.5rem' }}
                />
                ...
                {showClienteDropdown && (
                  <div className="absolute z-10 w-full bg-[#1a1a1a] border border-t-0 border-[#D4AF37]/30 rounded-b-lg max-h-40 overflow-y-auto shadow-2xl">
                    {clientes.filter(c => c.nome.toLowerCase().includes(walkinClienteSearch.toLowerCase())).map(c => (
                      <div 
                        key={c.id} 
                        className="px-4 py-2 hover:bg-[#D4AF37]/10 cursor-pointer text-[#E5E5E5] text-sm flex items-center gap-2"
                        onClick={() => {
                           setWalkinClienteId(c.id);
                           setWalkinClienteSearch(c.nome);
                    ...
                    {clientes.filter(c => c.nome.toLowerCase().includes(walkinClienteSearch.toLowerCase())).length === 0 && (
              ...
              <div>
                <label className="text-xs font-semibold text-[#E5E5E5]/80 uppercase block mb-2">Serviços Selecionados</label>
                <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2 border-y border-[#D4AF37]/10 py-2">
                  {servicos.map(s => (
                    <label key={s.id} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-[#1a1a1a] rounded transition-colors border border-transparent hover:border-[#D4AF37]/10">
                      <input 
                        type="checkbox" 
                        checked={walkinServicosIds.includes(s.id)}
                        onChange={() => toggleServico(s.id)}
                        className="w-4 h-4 accent-[#D4AF37] bg-[#1a1a1a] border-[#D4AF37]/30"
                      />
                      <div className="flex-1 text-sm text-[#E5E5E5]">{s.nome}</div>
                      <div className="text-xs text-[#D4AF37]">{s.duracaoMinutos}m</div>
                    </label>
                  ))}
                </div>
              </div>
