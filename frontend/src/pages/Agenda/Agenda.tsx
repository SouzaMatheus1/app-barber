...
export function Agenda() {
  ...
  const submitNovoAgendamento = async (e: React.FormEvent) => {
    ...
    const obsVal = novoAgendamentoClienteSearch ? `Avulso: ${novoAgendamentoClienteSearch}` : 'Cliente Avulso';
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end border-b border-[var(--color-primary)]/20 pb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-3 w-full lg:w-auto">
          ...
          <input 
            ...
            className="bg-[var(--color-background)] text-[var(--color-primary)] border border-[var(--color-primary)]/30 rounded-lg outline-none px-4 py-2 font-bold cursor-pointer"
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <button 
          ...
          className="bg-[var(--color-background)] border border-red-500/50 hover:bg-red-500/10 text-red-500 font-bold px-5 py-2.5 rounded-lg flex items-center gap-2 transition-colors uppercase text-sm tracking-widest"
        >
      ...
      {/* Linha do Tempo */}
      <div className="bg-[var(--color-background)] rounded-xl border border-[var(--color-primary)]/20 shadow-lg p-3 sm:p-6 min-h-[60vh] relative overflow-hidden">
        {loading ? (
             <span className="text-[var(--color-primary)] animate-pulse flex justify-center mt-20">Atualizando grade...</span>
        ) : (
          <div className="relative">
            <div className="absolute left-[70px] sm:left-[80px] top-0 bottom-0 w-px bg-[var(--color-primary)]/10" />
                ...
                return (
                  <div key={hour} className="flex min-h-[80px] relative group">
                    <div className="w-[70px] sm:w-[80px] flex-shrink-0 flex items-start pt-2">
                        <span className="text-xs sm:text-sm font-bold text-[var(--color-text)]/40 group-hover:text-[var(--color-primary)]/70 transition-colors">
                            {timeString}
                        </span>
                    </div>

                    <div className="flex-1 border-b border-[var(--color-primary)]/5 pb-4 pl-2 sm:pl-4 pt-2 flex flex-col gap-3">
                        {itemsDaHora.length === 0 ? (
                            <div className="text-xs text-[var(--color-text)]/10 mt-1 hidden group-hover:block">Livre</div>
                        ) : (
                            itemsDaHora.map(ag => {
                                ...
                                return (
                                    <div key={ag.id} className={`
                                        ...
                                        ${isCancelado ? 'bg-red-900/10 border-red-500/20 opacity-60' 
                                        : isConcluido ? 'bg-green-900/10 border-green-500/20 opacity-60' 
                                        : 'bg-[var(--color-surface)] border-[var(--color-primary)]/30 shadow-md'}
                                    `}>
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-2">
                                                <strong className={
                                                    isCancelado ? 'text-red-500/70 line-through text-sm sm:text-base' 
                                                    : isConcluido ? 'text-green-500/70 line-through text-sm sm:text-base' 
                                                    : 'text-[var(--color-primary)] text-sm sm:text-base'
                                                }>
                                                    {new Date(ag.dataHoraInicio).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})} - {new Date(ag.dataHoraFim).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}
                                                </strong>
                                                <span className={`text-[9px] sm:text-[10px] px-2 py-0.5 rounded tracking-widest font-bold ${
                                                    isCancelado ? 'bg-red-500/20 text-red-500' 
                                                    : isConcluido ? 'bg-green-500/20 text-green-500' 
                                                    : 'bg-[var(--color-primary)]/20 text-[var(--color-primary)]'
                                                }`}>
                                                    {statusMap[ag.status] || ag.status}
                                                </span>
                                            </div>
                                            
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-[var(--color-text)]/80">
                                                <div className="flex items-center gap-2">
                                                  <User size={14} className="text-[var(--color-text)]/40"/>
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                {ag.servicos && ag.servicos.length > 0 ? (
                                                  ag.servicos.map((s, idx) => (
                                                      <span key={idx} className="text-[10px] sm:text-xs bg-[var(--color-background)] border border-[var(--color-primary)]/10 px-2 py-1 rounded text-[var(--color-text)]/60 flex items-center gap-1">
                                                         {s.item.nome} <span className="opacity-50 ml-0.5">({s.item.duracaoMinutos || 30}m)</span>
                                                      </span>
                                                  ))
                                                ) : (
                                                  <span className="text-[10px] text-[var(--color-text)]/40 italic">Serviços serão definidos no pagamento</span>
                                                )}
                                            </div>
                                        </div>

                                        {!isConcluido && !isCancelado && (
                                            <div className="flex items-center gap-2 border-t border-[var(--color-primary)]/10 pt-3 md:pt-0 md:border-none">
                                                {ag.status !== 'EM_ANDAMENTO' && (
                                                  <button 
                                                    onClick={() => handleUpdateStatus(ag.id, 'EM_ANDAMENTO')}
                                                    className="px-2 sm:px-3 py-1.5 bg-[var(--color-primary)]/10 text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-[var(--color-background)] flex items-center gap-2 rounded text-[10px] sm:text-xs tracking-wider transition-colors font-bold"
                                                  >
      ...
      {isBlockModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <form onSubmit={submitBlock} className="bg-[var(--color-surface)] border border-red-500/30 rounded-xl p-6 w-full max-w-md shadow-2xl relative">
            <button type="button" onClick={() => setIsBlockModalOpen(false)} className="absolute top-4 right-4 text-[var(--color-text)]/50 hover:text-[var(--color-text)]">
              <X size={24} />
            </button>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase">Data e Hora do Início</label>
                <div className="flex gap-2.5 mt-1">
                  <input required type="date" value={blockData} onChange={e => setBlockData(e.target.value)} className="w-[160px] bg-[var(--color-background)] text-[var(--color-text)] border border-red-500/30 focus:border-red-500 rounded-lg px-4 py-2 outline-none" style={{ colorScheme: 'dark' }} />
                  <input required type="time" value={blockHora} onChange={e => setBlockHora(e.target.value)} className="flex-1 bg-[var(--color-background)] text-[var(--color-text)] border border-red-500/30 focus:border-red-500 rounded-lg px-4 py-2 outline-none" style={{ colorScheme: 'dark' }} />
                </div>
              </div>

              <div>
                <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase">Duração do Bloqueio (em minutos)</label>
                <input required type="number" step="10" min="10" value={blockDuracao} onChange={e => setBlockDuracao(Number(e.target.value))} className="w-full mt-1 bg-[var(--color-background)] text-[var(--color-text)] border border-red-500/30 focus:border-red-500 rounded-lg px-4 py-2 outline-none" />
              </div>

              <div>
                <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase">Motivo</label>
                <input type="text" value={blockMotivo} onChange={e => setBlockMotivo(e.target.value)} placeholder="Ex: Horário de Almoço" className="w-full mt-1 bg-[var(--color-background)] text-[var(--color-text)] border border-red-500/30 focus:border-red-500 rounded-lg px-4 py-2 outline-none" />
              </div>
            </div>
      ...
      {isNovoAgendamentoModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <form onSubmit={submitNovoAgendamento} className="bg-[var(--color-surface)] border border-[var(--color-primary)]/30 rounded-xl p-6 w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button type="button" onClick={() => setIsNovoAgendamentoModalOpen(false)} className="absolute top-4 right-4 text-[var(--color-text)]/50 hover:text-[var(--color-text)]">
              <X size={24} />
            </button>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase">Data e Hora</label>
                <div className="flex gap-2.5 mt-1">
                  <input required type="date" value={novoAgendamentoData} onChange={e => setNovoAgendamentoData(e.target.value)} className="w-[160px] bg-[var(--color-background)] text-[var(--color-text)] border border-[var(--color-primary)]/30 focus:border-[var(--color-primary)] rounded-lg px-4 py-2 outline-none" style={{ colorScheme: 'dark' }} />
                  <input required type="time" value={novoAgendamentoHora} onChange={e => setNovoAgendamentoHora(e.target.value)} className="flex-1 bg-[var(--color-background)] text-[var(--color-text)] border border-[var(--color-primary)]/30 focus:border-[var(--color-primary)] rounded-lg px-4 py-2 outline-none" style={{ colorScheme: 'dark' }} />
                </div>
              </div>

              <div className="relative">
                <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase">Nome do Cliente</label>
                <input 
                  type="text" 
                  ...
                  className={`w-full mt-1 bg-[var(--color-background)] border border-[var(--color-primary)]/30 px-4 py-2 outline-none transition-colors ${novoAgendamentoClienteId !== '' ? 'text-[var(--color-primary)] font-bold' : 'text-[var(--color-text)]'}`} 
                ...
                {showClienteDropdown && (
                  <div className="absolute z-10 w-full bg-[var(--color-background)] border border-t-0 border-[var(--color-primary)]/30 rounded-b-lg max-h-40 overflow-y-auto shadow-2xl">
                    {clientes.filter(c => c.nome.toLowerCase().includes(novoAgendamentoClienteSearch.toLowerCase())).map(c => (
                      <div 
                        key={c.id} 
                        className="px-4 py-2 hover:bg-[var(--color-primary)]/10 cursor-pointer text-[var(--color-text)] text-sm flex items-center gap-2"
                        onClick={() => {
                      ...
                      >
                        <User size={14} className="text-[var(--color-primary)]" /> {c.nome}
                      </div>
                    ))}
                    {clientes.filter(c => c.nome.toLowerCase().includes(novoAgendamentoClienteSearch.toLowerCase())).length === 0 && (
                      <div className="px-4 py-2 text-[var(--color-text)]/40 text-sm">Pressione Enter para salvar como avulso.</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <input required type="number" step="5" min="10" value={novoAgendamentoDuracao} onChange={e => setNovoAgendamentoDuracao(Number(e.target.value))} className="w-full mt-1 bg-[var(--color-background)] text-[var(--color-text)] border border-[var(--color-primary)]/30 focus:border-[var(--color-primary)] rounded-lg px-4 py-2 outline-none" />
              </div>
            </div>

            <button type="submit" className="w-full mt-8 bg-[var(--color-primary)] hover:bg-[var(--color-secondary)] text-[var(--color-background)] font-bold py-3 rounded-lg uppercase tracking-wider transition-colors">
              Agendar Serviço
            </button>
          </form>
        </div>
