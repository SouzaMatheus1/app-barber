      ...
      {isEditing && (
        <form onSubmit={handleCreateOrUpdate} className="bg-[var(--color-surface)] rounded-xl p-6 border border-[var(--color-primary)]/30 shadow-lg space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase tracking-wider">E-mail (Login)</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase tracking-wider">
                Senha {editingId && <span className="text-[var(--color-text)]/40 text-[10px] ml-1">(Opcional para ignorar alteração)</span>}
              </label>
              <input
                type="password"
                required={!editingId}
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-[var(--color-text)]/80 uppercase tracking-wider">Perfil</label>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[var(--color-background)]">
                ...
                <th className="py-4 px-6 text-[var(--color-text)]/70 font-semibold text-sm uppercase tracking-wider">E-mail</th>
                <th className="py-4 px-6 text-[var(--color-text)]/70 font-semibold text-sm uppercase tracking-wider">Perfil</th>
            ...
            <tbody className="divide-y divide-[var(--color-primary)]/10">
              {profissionais.length === 0 && !loading ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-[var(--color-text)]/50">Nenhum profissional cadastrado.</td>
                </tr>
              ) : (
                profissionais.map((profissional) => (
                  <tr key={profissional.id.toString()} className="hover:bg-[var(--color-primary)]/5 transition-colors group">
                    <td className="py-4 px-6 text-[var(--color-text)] font-medium">{profissional.nome}</td>
